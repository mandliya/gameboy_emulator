...
bool ByteRegister::set_bit_to(u8 bit, bool set)
{
    return Bitwise::set_bit_to(val, bit, set);
}

bool ByteRegister::increment()
{
    val += 1;
}

bool ByteRegister::decrement()
{
    val -= 1;
}
...
{
    // Only high nibble carry valid flag bits.
    val = new_value & 0xF0;
}
...
void FlagRegister::set_flag_substract(bool set)
{
    set_bit_to(6, set);
}
...
bool FlagRegister::flag_substract() const
{
    return check_bit(6);
}
...
u8 FlagRegister::flag_substract_value() const
{
    return static_cast<u8>(flag_substract() ? 1 : 0);
}
